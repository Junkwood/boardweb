...
async function addReplyFnc(e) {
	...
	try {
		...
		if (result.retCode == 'OK') {
			...
			let realend = Math.ceil(result.totalCount / 5);
			page = realend
			replyList(page);
			pageList();
		} else {
...
//목록함수
async function replyList(rpage = 1) {
	try {
		let resolve = await fetch('replyList.do?bno=' + bno + '&page=' + rpage, {
			method: 'get'
		})
		let result = await resolve.json();
...
async function pageList() {
	try{
	let resolve = await fetch('getTotal.do?bno=' + bno, {
		method: 'get'
	})
	let result = await resolve.json();
